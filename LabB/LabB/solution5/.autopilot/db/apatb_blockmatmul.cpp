...
long __xlx_apatb_param_Arows_stream_buf_final_size;
{
  ...
  for (long i = 0; i < __xlx_apatb_param_Arows_stream_buf_final_size; ++i)((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->read();
long __xlx_apatb_param_Bcols_stream_buf_final_size;
{
  ...
  for (long i = 0; i < __xlx_apatb_param_Bcols_stream_buf_final_size; ++i)((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->read();
#ifdef USE_BINARY_TV_FILE
{
transaction<32> tr(256);
aesl_fh.read(AUTOTB_TVOUT_PC_ABpartial, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<4>((char*)__xlx_apatb_param_ABpartial, 256);
}
#else
{
      static ifstream rtl_tv_out_file;
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > ABpartial_pc_buffer(256);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 256; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_ABpartial)[j*4+0] = ABpartial_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_ABpartial)[j*4+1] = ABpartial_pc_buffer[i].range(15, 8).to_int64();
((char*)__xlx_apatb_param_ABpartial)[j*4+2] = ABpartial_pc_buffer[i].range(23, 16).to_int64();
((char*)__xlx_apatb_param_ABpartial)[j*4+3] = ABpartial_pc_buffer[i].range(31, 24).to_int64();
}}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
std::vector<__cosim_s64__> __xlx_apatb_param_Arows_stream_buf;
{
  while (!((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->empty())
    __xlx_apatb_param_Arows_stream_buf.push_back(((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->read());
  for (int i = 0; i < __xlx_apatb_param_Arows_stream_buf.size(); ++i)
    ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->write(__xlx_apatb_param_Arows_stream_buf[i]);
  }
long __xlx_apatb_param_Arows_stream_buf_size = ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->size();
std::vector<__cosim_s64__> __xlx_apatb_param_Bcols_stream_buf;
{
  while (!((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->empty())
    __xlx_apatb_param_Bcols_stream_buf.push_back(((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->read());
  for (int i = 0; i < __xlx_apatb_param_Bcols_stream_buf.size(); ++i)
    ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->write(__xlx_apatb_param_Bcols_stream_buf[i]);
  }
long __xlx_apatb_param_Bcols_stream_buf_size = ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->size();
unsigned __xlx_offset_byte_param_ABpartial = 0;
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_ABpartial, 'b');
transaction<32> tr(256);
  __xlx_offset_byte_param_ABpartial = 0*4;
  if (__xlx_apatb_param_ABpartial) {
tr.import<4>((char*)__xlx_apatb_param_ABpartial, 256, 0);
  }
aesl_fh.write(AUTOTB_TVIN_ABpartial, tr.p, tr.tbytes);
}

  tcl_file.set_num(256, &tcl_file.ABpartial_depth);
#else
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_ABpartial + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVIN_ABpartial, s);
}
}
}
...
long __xlx_apatb_param_Arows_stream_buf_final_size = __xlx_apatb_param_Arows_stream_buf_size - ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Arows)->size();
...
long __xlx_apatb_param_Bcols_stream_buf_final_size = __xlx_apatb_param_Bcols_stream_buf_size - ((hls::stream<__cosim_s64__>*)__xlx_apatb_param_Bcols)->size();
...
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVOUT_ABpartial, 'b');
transaction<32> tr(256);
  __xlx_offset_byte_param_ABpartial = 0*4;
  if (__xlx_apatb_param_ABpartial) {
tr.import<4>((char*)__xlx_apatb_param_ABpartial, 256, 0);
  }
aesl_fh.write(AUTOTB_TVOUT_ABpartial, tr.p, tr.tbytes);
}
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_ABpartial + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVOUT_ABpartial, s);
}
}
}
