...
#include "blockmatmul_AB_RAM_AUTO_1R1W.h"

using namespace std;
using namespace sc_core;
using namespace sc_dt;

blockmatmul_AB_RAM_AUTO_1R1W::~blockmatmul_AB_RAM_AUTO_1R1W() {
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_i_q0() {
    i_q0.write(buf_q0[prev_iptr.read()].read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_t_q0() {
    t_q0.write(buf_q0[prev_tptr.read()].read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_i_q1() {
    i_q1.write(buf_q1[prev_iptr.read()].read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_t_q1() {
    t_q1.write(buf_q1[prev_tptr.read()].read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_push_buf() {
    push_buf.write(i_ce.read() & i_write.read() & full_n.read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_pop_buf() {
    pop_buf.write(t_ce.read() & t_read.read() & empty_n.read());
}

void blockmatmul_AB_RAM_AUTO_1R1W::proc_iptr() {
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_tptr() {
    if (reset.read() == SC_LOGIC_1) {
        tptr.write(0);
        prev_tptr.write(0);
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void blockmatmul_AB_RAM_AUTO_1R1W::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
