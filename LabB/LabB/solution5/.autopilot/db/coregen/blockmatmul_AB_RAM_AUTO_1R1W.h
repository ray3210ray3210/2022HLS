...
#ifndef _blockmatmul_AB_RAM_AUTO_1R1W_H_
#define _blockmatmul_AB_RAM_AUTO_1R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "blockmatmul_AB_RAM_AUTO_1R1W_memcore.h"

SC_MODULE(blockmatmul_AB_RAM_AUTO_1R1W) {
    static const unsigned int DataWidth    = 512;
    static const unsigned int EnableWidth    = 64;
    ...
    sc_core::sc_in<  sc_dt::sc_lv<EnableWidth> >        i_we0;
    ...
    sc_core::sc_in<  sc_dt::sc_lv<EnableWidth> >        i_we1;
    ...
    sc_core::sc_in<  sc_dt::sc_lv<EnableWidth> >        t_we0;
    ...
    sc_core::sc_in<  sc_dt::sc_lv<EnableWidth> >        t_we1;
...
protected:
    ...
    // buffer instances
    blockmatmul_AB_RAM_AUTO_1R1W_memcore* buffer[BufferCount];

    // buffer signals
    sc_core::sc_signal< sc_dt::sc_logic >              buf_ce0[BufferCount];
    sc_core::sc_signal< sc_dt::sc_lv<EnableWidth> >              buf_we0[BufferCount];
    ...
    sc_core::sc_signal< sc_dt::sc_lv<EnableWidth> >              buf_we1[BufferCount];
...
public:
    ...
    SC_CTOR(blockmatmul_AB_RAM_AUTO_1R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "blockmatmul_AB_RAM_AUTO_1R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new blockmatmul_AB_RAM_AUTO_1R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "blockmatmul_AB_RAM_AUTO_1R1W.trace");
            // ports
