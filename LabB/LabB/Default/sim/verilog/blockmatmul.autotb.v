...
parameter LENGTH_Arows = 8;
parameter LENGTH_Bcols = 8;
parameter LENGTH_ABpartial = 1;
...
wire ready;
wire ready_wire;
wire [127 : 0] Arows_dout;
wire  Arows_empty_n;
wire  Arows_read;
wire [127 : 0] Bcols_dout;
wire  Bcols_empty_n;
wire  Bcols_read;
wire [511 : 0] ABpartial_i;
wire [511 : 0] ABpartial_o;
wire [31 : 0] it;
wire ap_start;
wire  ABpartial_o_ap_vld;
wire ap_done;
wire ap_ready;
wire ap_idle;
integer done_cnt = 0;
...
`AUTOTB_DUT `AUTOTB_DUT_INST(
    ...
    .ABpartial_o(ABpartial_o),
    .it(it),
    .ap_start(ap_start),
    .ABpartial_o_ap_vld(ABpartial_o_ap_vld),
    .ap_done(ap_done),
    .ap_ready(ap_ready),
    .ap_idle(ap_idle));
...
// The signal of port ABpartial_i
reg [511: 0] AESL_REG_ABpartial_i = 0;
assign ABpartial_i = AESL_REG_ABpartial_i;
always @(posedge AESL_clock)
begin
    if(ABpartial_o_ap_vld === 1)
        AESL_REG_ABpartial_i <= ABpartial_o;
end

initial begin : read_file_process_ABpartial
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVIN_ABpartial,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_ABpartial);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_ABpartial_i);
              if (ret != 1) begin
...
reg AESL_REG_ABpartial_o_ap_vld = 0;
// The signal of port ABpartial_o
reg [511: 0] AESL_REG_ABpartial_o = 0;
always @(posedge AESL_clock)
begin
    if(AESL_reset)
        AESL_REG_ABpartial_o = 0; 
    else if(ABpartial_o_ap_vld) begin
        AESL_REG_ABpartial_o <= ABpartial_o;
        AESL_REG_ABpartial_o_ap_vld <= 1;
    end
end 

initial begin : write_file_process_ABpartial
    integer fp;
    integer fp_size;
    integer err;
    integer ret;
    ...
    integer proc_rand;
    integer ABpartial_count;
    reg [1047:0] token;
    integer transaction_idx;
    reg [8 * 5:1] str;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVOUT_ABpartial_out_wrapc,"w");
    if(fp == 0) begin       // Failed to open file
    ...
    while (transaction_idx != AUTOTB_TRANSACTION_NUM) begin
        ...
        if(AESL_REG_ABpartial_o_ap_vld)  begin
          $fdisplay(fp,"0x%x", AESL_REG_ABpartial_o);
        AESL_REG_ABpartial_o_ap_vld = 0;
        end
