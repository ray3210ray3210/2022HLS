...
int main() {
   ...
   int fail = 0;
    hls::stream<blockvec> strm_matrix1("strm_matrix1");//name stream , easy to debug
    hls::stream<blockvec> strm_matrix2("strm_matrix2");
    blockvec strm_matrix1_element, strm_matrix2_element;//1x4
    blockmat block_out; //4x4
    DTYPE A[SIZE][SIZE], B[SIZE][SIZE]; //8x8
    DTYPE matrix_swout[SIZE][SIZE], matrix_hwout[SIZE][SIZE];   //8x8
        ...
        for(int it2 = 0; it2 < SIZE; it2 = it2 + BLOCK_SIZE) {
            ...
            for(int k = 0; k < SIZE; k++) {
                for(int i = 0; i < BLOCK_SIZE; i++) {  
                    if(it % (SIZE/BLOCK_SIZE) == 0) //reuse A 
